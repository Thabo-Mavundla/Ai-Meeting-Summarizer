...
def process_audio():
    ...
    try:
        ...
        return jsonify({
            ...
            "target_language": target_lang  # Return the language used for refer
        })
...
def delete_history():
    ...
    try:
        ...
        updated_entries = [
            ...
            if not (entry['filename'] == filename and entry['timestamp'] == timestamp)
        ]
...
def history():
    try:
        with open(HISTORY_FILE, 'r') as f:
            ...
            history_entries = [json.loads(line) for line in f.readlines() if line.strip()]
        # Reverse to show newest first and convert to list (reversed returns an iterator)
        return render_template('history.html', history=list(reversed(history_entries)))
    except FileNotFoundError:
